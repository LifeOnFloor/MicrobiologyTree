/**
 * script_toggle.js
 * D3.js v7を使用して折りたたみ可能なツリーグラフを描画します。
 * 
 * このコードは、階層構造をもつデータを使用して、ツリー型のグラフを描画します。
 * 
 * グラフ詳細：
 *     - ノードの円をクリックすると、ノードが展開/折りたたみされます。
...
class Node {
    ...
    nodeEnterUpdate(node, source) {
        ...
        nodeEnter.append("circle")
            ...
            .attr("r", 10)
            .attr("role", "button")
            .attr("aria-label", d => `${d.data.name}を展開/折りたたみ`)
            .attr("tabindex", 0)
            .on("click", (event, d) => this.toggleCollapse(event, d))
            .on("keypress", (event, d) => {
                if (event.key === 'Enter' || event.key === ' ') {
                    event.preventDefault();
                    this.toggleCollapse(event, d);
                }
            });
